...
final onTimePerformanceProvider = Provider<List<OnTimeDataPoint>>((ref) {
  ...
  // Only routes with a valid scheduled_arrival_time
  ...
  // Group alighted records by (routeId, date) → latest timestamp
  ...
  // For each (routeId, date) pair determine on-time, then aggregate by date
  ...
  for (final entry in latestAlighted.entries) {
    ...
    final lateBy = actualMins - scheduledMins;
    final isOnTime = lateBy <= _gracePeriodMinutes;
